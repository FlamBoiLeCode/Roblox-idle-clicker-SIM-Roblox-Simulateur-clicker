-- ClickHandler.lua V4.3 TikTok
-- LocalScript dans StarterPlayerScripts
-- Features : Base + Shop (F1) + NumberFormatter (F4.1) + Animation clic (F4.2) + Sons (F4.3)
...
-- Écouter le résultat du clic depuis le serveur
...
-- F4.2 : MouseButton1Click avec TweenService (rebond smooth) + F4.3 : Son clic
clickButton.MouseButton1Click:Connect(function()
	clickEvent:FireServer()
	clickSound:Play() -- F4.3
...
local function createUpgradeRow(upgradeInfo, index)
	...
	-- F4.1 : Bouton acheter avec NumberFormatter
